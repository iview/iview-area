...
```
# use 使用
在main.js中写入下面的代码   write follow code in main.js  
```
    import iviewArea from 'iview-area';
    import Vue from 'vue';
    Vue.use(iviewArea);
```
接下来，你就可以在页面中使用iview-area了   and then, you can use iview-area anywhere  
```
<template>
...
data-type|返回数据的类型，'all':城市编码和名称，'code':只返回编码，'name':只返回名称|String|all
searchable|是否可搜索，添加该属性则点击选择器后可输入名称搜索|-|false
