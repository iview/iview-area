...
> **default属性补充说明:**
>
> - 传入名称数组，若所设地方名称未找到或地方所属关系不对，则显示该等级列表中第一个地方，若数组地方个数少于城市及联选择器的等级数目，则后面缺省的地方名默认已列表中第一个地方显示；且如果设置了数组且不为空，则每次选择一个等级的地方后下面级别的选择器的列表都会更新，且默认选中的为对应列表中第一个地方
>   ```
>   ex: ['河北省', '长春市']  在级联选择器中显示  ['河北省', '石家庄市']
>   ```
> - 传入编号数组，若所设编号未找到对应地方或地方所属关系不对，则显示该等级列表中第一个地方，规则同上
>   ```
...
=======
>>>>>>> 58aaa69d48a66d4e47372fbe1f41ee486a69ab98
##### 补充说明：
...
>   ex: ['河北省', '长春市']  在级联选择器中显示  ['河北省', '石家庄市']
>   ```
> - 传入编号数组，若所设编号未找到对应地方或地方所属关系不对，则显示该等级列表中第一个地方，规则同上
>   ```
>   ex: ['130000', '120100']  在级联选择器中显示  ['河北省', '石家庄市']
>   ```
> - 传入空数组``[]``或``undefined``或``null``，或无``default``，或设置了``default``未设置值，则不显示默认值，且每选择一个级别的地方后，只更新下一个级别的地方列表，且没有默认选中值
